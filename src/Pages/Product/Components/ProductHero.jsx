      ...
      <div className={`fixed top-0 left-0 h-full w-[320px] sm:w-[380px] bg-white z-[101] shadow-2xl transform transition-transform duration-500 ease-out flex flex-col ${isCartOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="p-5 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-black text-stone-800">سبد خرید</h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          <div className="flex gap-4 p-4 rounded-2xl border border-stone-100 bg-white shadow-sm relative group">
            <div className="w-20 h-20 rounded-xl bg-stone-100 overflow-hidden flex-shrink-0">
                <img src={product.image} alt={product.title} className="w-full h-full object-cover" />
            </div>
            <div className="flex-1 min-w-0">
              <h3 className="text-sm font-bold text-stone-800 truncate">{product.title}</h3>
              <div className="flex items-center justify-between mt-3">
                 <div className="flex items-center gap-3 bg-stone-50 px-2 py-1 rounded-lg">
                    <button onClick={increaseCount} className="text-stone-400">+</button>
                    <span className="text-xs font-black">{count}</span>
                    <button onClick={decreaseCount} className="text-stone-400">−</button>
                 </div>
                 <div className="text-left">
                    <span className="text-sm font-black text-primary">{getPrice().toLocaleString('fa-IR')}</span>
        ...
        <div className="p-6 border-t border-gray-100 bg-gray-50/30">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="text-left">
              <span className="text-2xl font-black text-primary">{totalPrice.toLocaleString('fa-IR')}</span>
              <span className="text-xs text-stone-400 mr-1">تومان</span>
            </div>
          </div>
          <button className="w-full py-4 bg-[#9EAD8C] text-white rounded-2xl font-bold text-sm shadow-lg hover:bg-[#8A9B78] transition-all">
