...
import SectionHeader from './SectionHeader'; // ← اصلاح مسیر
          ...
          {faqs.map((faq) => (
            <div 
              ...
              className={`group bg-white rounded-2xl overflow-hidden transition-all duration-300 border ${
                openId === faq.id 
                  ? 'shadow-2xl border-[#9EAD8C]/40' 
                  : 'shadow-md hover:shadow-lg border-gray-100 hover:border-[#9EAD8C]/20'
              }`}
            >
              <button 
                onClick={() => setOpenId(openId === faq.id ? null : faq.id)}
                className="w-full flex justify-between items-center p-5 text-right transition-all group"
              >
                <div className="flex items-center gap-3 flex-1">
                  ...
                  <span className={`text-sm md:text-base font-medium transition-colors ${
                    openId === faq.id ? 'text-[#8A9A7B]' : 'text-[#4A3728]'
                  }`}>
                    {faq.question}
                  </span>
                </div>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 flex-shrink-0 ${
                  openId === faq.id 
                    ...
                    : 'bg-gray-100 text-[#9EAD8C] group-hover:bg-[#9EAD8C]/10'
                }`}>
                  <ChevronDown size={20} />
                </div>
              </button>
              <div className={`transition-all duration-300 overflow-hidden ${
                ...
                <div className={`px-5 pb-5 pt-0 text-sm text-gray-600 leading-relaxed border-t ${getCategoryColor(faq.category)}`}>
                  <div className="flex items-start gap-4 pt-4">
                    <div className="w-1 h-12 bg-gradient-to-b from-[#9EAD8C] to-[#8A9A7B] rounded-full flex-shrink-0" />
