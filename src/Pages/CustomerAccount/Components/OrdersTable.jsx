...
const OrdersTable = ({ orders, onViewDetails }) => {
  const statusMap = { delivered: { label: 'تحویل شده', color: COLORS.success, bg: '#e8f5ee' }, processing: { label: 'در حال ارسال', color: COLORS.warm, bg: '#fef0e8' }, pending: { label: 'در انتظار', color: COLORS.primaryLight, bg: '#f0f3ec' } };
  ...
  return (
    <div className="w-full overflow-x-auto rounded-lg" style={{ WebkitOverflowScrolling: 'touch' }}>
      <table className="w-full min-w-[640px] text-sm md:min-w-0">
        <thead><tr style={{ background: COLORS.primaryBg }}>
        ...
        <tbody>
          {orders.map(order => {
            ...
            return <tr key={order.id} className="border-b last:border-b-0 transition-colors hover:bg-[#fafcf8]" style={{ borderColor: COLORS.border }}>
              ...
              <td className="hidden whitespace-nowrap px-3 py-2.5 text-xs sm:table-cell"><span className={`rounded-full px-2 py-0.5 ${order.paid === 'بله' ? 'text-white' : ''}`} style={order.paid === 'بله' ? { background: COLORS.success } : { color: COLORS.primaryLight, border: `1px solid ${COLORS.border}` }}>{order.paid}</span></td>
              <td className="hidden whitespace-nowrap px-3 py-2.5 text-xs sm:table-cell"><span className="rounded-full px-2 py-0.5" style={{ color: status.color, backgroundColor: status.bg, border: `1px solid ${status.color}20` }}>{order.delivered}</span></td>
              <td className="whitespace-nowrap px-3 py-2.5 text-center"><button onClick={() => onViewDetails(order)} className="rounded-lg px-3 py-1.5 text-xs text-white transition-all hover:shadow-md active:scale-95" style={{ background: `linear-gradient(135deg, ${COLORS.primary}, ${COLORS.primaryDark})` }}>جزئیات</button></td>
            </tr>;
          })}
        </tbody>
      </table>
